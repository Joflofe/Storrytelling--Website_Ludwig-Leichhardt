...
const allLayers = map.getStyle().sources;
console.log('Sources:', JSON.stringify(allLayers));
const testLayers = ['landuse', 'landcover', 'water', 'waterway', 'boundaries', 'places', 'contours', 'buildings', 'roads', 'transit', 'earth', 'pois'];
testLayers.forEach(layer => {
  const f = map.querySourceFeatures('protomaps', { sourceLayer: layer });
  console.log(layer + ':', f.length, f.length > 0 ? Object.keys(f[0].properties) : '');
});
  }
